...
CREATE TABLE IF NOT EXISTS event_feed (
    event_id BIGINT GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY, --id события
    user_id BIGINT REFERENCES users (user_id) ON DELETE CASCADE, --id пользователя
    type_id BIGINT REFERENCES event_type  (type_id),
    operation_id BIGINT REFERENCES event_operation (operation_id),
